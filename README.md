![](cpkmeh66n5rmr0cwbe6bnvmdjm_result_0.png)
# kuangkuang
一款具有签到与群聊功能的安卓软件/An android app which contins the function of attendance and chat
## 日志
### 25.10.10
初步开发聊天群聊与好友功能
### 25.11.17
开发了ai审核与jwt校验功能
### 25.12.7
实现龙王评选
### 25.12.24
增加了邮箱验证功能，修复了无群聊用户获取群聊列表会报错的bug
## TODO
1. 实现通知、签到等功能
2. 完善群聊功能，实现对群聊成员的查询与群聊人数的显示
3. 实现对头像的更换（暂定）
## Tip
1. 代码中已经设有TODO,需根据提示进行填写IP地址等内容
2. Android代码在app，后端代码在backend

---

# kuangkuang
An Android app featuring attendance tracking and group chat functionality
## Log
### 25.10.10
Initial development of chat group and friend features
### 25.11.17
Developed AI review and JWT verification features
### 25.12.7
Implemented Dragon King selection
### 25.12.24
Added email verification functionality. Fixed a bug where users without group chats would encounter errors when retrieving group chat lists.
## TODO
1. Implement notifications, check-in, and other functions
2. Enhance group chat functionality: enable member queries and display group size
3. Implement profile picture changes (tentative)
## Tip
1. TODO tags are already present in the code; fill in details like IP addresses as prompted
2. The code od front end is in the app branch,back end is in the back end banch.


